...
class Order extends \Magento\Framework\App\Action\Action
{
     ...
     */
    public function __construct(Context $context, BidziPayment $payment, \Psr\Log\LoggerInterface $logger_interface, Cart $cart, \Magento\Quote\Model\QuoteManagement $quoteManagement)
    {
    ...
    public function execute()
    {
        ...
        try {
            ...
            $bidzi_order = $this->payment->createBidziOrder($purchase_data);
            $this->logger->debug('bidzi_order ====> ' . json_encode($bidzi_order));


            // Create Bidzi Payment
            $base_url = $this->payment->getBaseUrlStore();
            $payment_data = $this->getPaymentData($bidzi_order['order_id'], $post['device_session_id'], $post['payment_method_id'], $base_url);
            $this->logger->debug('payment_data ====> ' . json_encode($payment_data));
            $bidzi_payment = $this->payment->createBidziPayment($quote->getId(), $payment_data);
            $this->logger->debug('bidzi_payment ====> ' . json_encode($bidzi_payment));
        ...
        $payment_request = [
            ...
            'payment_source' => [
                ...
                'settings' => [
                    ...
                    'redirection_url' => [
                        'completed_redirect_url' => $base_url . 'bidzi/payment/success',
