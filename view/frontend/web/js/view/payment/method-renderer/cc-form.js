/**
 * Bidzi_Cards Magento JS component
 *
 * @category    Bidzi
 * @package     Bidzi_Cards
...
define([
  ...
  var is_sandbox = window.checkoutConfig.payment.bidzi_credentials.is_sandbox === "0" ? false : true;
  var merchant_id = window.checkoutConfig.payment.bidzi_credentials.merchant_id;
  var public_key = window.checkoutConfig.payment.bidzi_credentials.public_key;
  ...
  var bidzi = initOrkestaPay({ merchant_id, public_key, is_sandbox });
  console.log("bidzi.js is ready!");
  ...
  return Component.extend({
    defaults: {
      template: "Bidzi_Cards/payment/bidzi-form",
    },
    ...
    preparePayment: async function () {
      ...
      var year = $("#Bidzi_Cards_expiration_yr").val();
      var holder_name = this.getCustomerFullName();
      var card = $("#Bidzi_Cards_cc_number").val();
      var cvc = $("#Bidzi_Cards_cc_cid").val();
      var month = $("#Bidzi_Cards_expiration").val();
      ...
      var bidzi_card = await bidzi.createCard();
      var payment_method = await bidzi_card.createToken(payment_method_data);

      var payment_method_id = payment_method.payment_method_id;
      var device_session_id = $("#device_session_id").val();

      $("#bidzi_token").val(payment_method_id);
      ...
      $.post(create_order_url, { payment_method_id, device_session_id })
        .done((response) => {
          ...
          if (response.status === "COMPLETED") {
            $("#bidzi_payment_id").val(payment_id);
    ...
    getData: function () {
      return {
        ...
        additional_data: {
          ...
          bidzi_token: $("#bidzi_token").val(),
          device_session_id: $("#device_session_id").val(),
          payment_id: $("#bidzi_payment_id").val(),
        },
      };
    },
    validate: function () {
    ...
    setDeviceSessionId: async function (bidzi) {
      try {
        const { device_session_id } = await bidzi.getDeviceInfo();
        console.log("setDeviceSessionId", device_session_id);
        $("#device_session_id").val(device_session_id);
      } catch (err) {
