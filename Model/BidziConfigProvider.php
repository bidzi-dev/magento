...
class BidziConfigProvider implements ConfigProviderInterface
{
    /**
    ...
    public function getConfig()
    {
        ...
        foreach ($this->methodCodes as $code) {
            if ($this->methods[$code]->isAvailable()) {
                ...
                $config['payment']['bidzi_credentials'] = array("merchant_id" => $this->payment->getMerchantId(), "public_key" => $this->payment->getPublicKey(), "is_sandbox"  => $this->payment->isSandbox());
                ...
                $config['payment']['create_order_url'] = $this->payment->getBaseUrlStore() . '/bidzi/payment/order';
                $config['payment']['complete_3ds_url'] = $this->payment->getBaseUrlStore() . '/bidzi/payment/complete';
