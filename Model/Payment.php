...
/**
 * Bidzi_Cards payment method model
 *
 * @category    Bidzi
 * @package     Bidzi_Cards
...
use Bidzi\Cards\Model\Utils\BidziRequest;
...
class Payment extends \Magento\Payment\Model\Method\Cc
{
    ...
    protected $bidziCustomerFactory;
     ...
     * @param BidziRequest $bidziRequest
     */
    public function __construct(
        ...
        $this->bidziRequest = $bidziRequest;
    }
    ...
    public function assignData(\Magento\Framework\DataObject $data)
    {
        ...
        $infoInstance->setAdditionalInformation('bidzi_token', isset($additionalData['bidzi_token']) ? $additionalData['bidzi_token'] : null);
    ...
    public function refund(\Magento\Payment\Model\InfoInterface $payment, $amount)
    {
        ...
        $bidzi_payment_id = str_replace('-refund', '', $trx_id);

        $this->logger_interface->debug('#refund', ['$bidzi_payment_id' => $bidzi_payment_id, '$order_id' => $order->getIncrementId(), '$status' => $order->getStatus(), '$amount' => $amount]);
        ...
        try {
            ...
            $this->createBidziRefund($bidzi_payment_id, $refundData);
        } catch (\Exception $e) {
    ...
    public function processCapture(\Magento\Payment\Model\InfoInterface $payment, $amount)
    {
        ...
        $payment_method_id = $this->getInfoInstance()->getAdditionalInformation('bidzi_token');
        ...
        try {
            // Obtiene el detalle del pago de Bidzi
            $bidziPayment = $this->getBidziPayment($payment_id);
            $this->logger_interface->debug('bidziPayment ====> ', $bidziPayment);

            $payment->setTransactionId($payment_id);

            if ($bidziPayment['status'] === 'COMPLETED') {
    ...
    public function createBidziOrder($order_request)
    {
        $credentials = ['client_id' => $this->client_id, 'client_secret' => $this->client_secret, 'grant_type' => 'client_credentials'];
        $bidziOrder = $this->bidziRequest->make('/v1/orders', $this->is_sandbox, $credentials, "POST", $order_request);
        return $bidziOrder;
    }

    public function createBidziPayment($order_id, $payment_request)
    {
        $credentials = ['client_id' => $this->client_id, 'client_secret' => $this->client_secret, 'grant_type' => 'client_credentials'];
        $idempotency_key = $order_id . '-' . time();
        $bidzi_payment = $this->bidziRequest->make('/v1/payments', $this->is_sandbox, $credentials, "POST", $payment_request, $idempotency_key);
        return $bidzi_payment;
    }

    public function getBidziPayment($payment_id)
    {
        $credentials = ['client_id' => $this->client_id, 'client_secret' => $this->client_secret, 'grant_type' => 'client_credentials'];
        $bidzi_payment = $this->bidziRequest->make("/v1/payments/" . $payment_id, $this->is_sandbox, $credentials);
        return $bidzi_payment;
    }

    public function getBidziPaymentMethod($payment_method_id)
    {
        $credentials = ['client_id' => $this->client_id, 'client_secret' => $this->client_secret, 'grant_type' => 'client_credentials'];
        $bidziPaymentMethod = $this->bidziRequest->make("/v1/payment-methods/" . $payment_method_id, $this->is_sandbox, $credentials);
        return $bidziPaymentMethod;
    }

    public function complete3DS($bidzi_payment_id)
    {
        $credentials = ['client_id' => $this->client_id, 'client_secret' => $this->client_secret, 'grant_type' => 'client_credentials'];
        $idempotency_key = $bidzi_payment_id;
        $complete_payment = $this->bidziRequest->make('/v1/payments/' . $bidzi_payment_id . '/complete', $this->is_sandbox, $credentials, "POST", [], $idempotency_key);
        return $complete_payment;
    }

    public function createBidziRefund($bidzi_payment_id, $data)
    {
        $credentials = ['client_id' => $this->client_id, 'client_secret' => $this->client_secret, 'grant_type' => 'client_credentials'];
        $idempotency_key = $bidzi_payment_id . '-' . time();
        $complete_payment = $this->bidziRequest->make('/v1/payments/' . $bidzi_payment_id . '/refund', $this->is_sandbox, $credentials, "POST", $data, $idempotency_key);
        return $complete_payment;
    }
